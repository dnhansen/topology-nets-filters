...
% TODO decide on whether to use subordination or not
...
\begin{proofsec}
    \item[(a) $\implies$ (b)]
    For each $N \in \nhoods{f(x)}$ there is an $M \in \nhoods{x}$ such that $f(M) \subseteq N$. Since $\mynet$ is eventually in $M$, $f(\mynet) = f \circ \mynet$ is eventually in $f(M) \subseteq N$, showing that $f(\mynet) \to f(x)$. % TODO: or just use filters? What do I like best?
